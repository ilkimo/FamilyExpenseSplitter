...
fun calculateContributions(
    ...
    val allMonths =
        ...
        (couple.person1.installments +
            couple.person2.installments)
            .map { it.date }
    val uniqueMonths = allMonths.distinct().sorted()
    ...
    for (month in uniqueMonths) {
        ...
        val person1Costs = sumCostsByMonth(couple.person1.costs, month)
        val person2Costs = sumCostsByMonth(couple.person2.costs, month)

        val person1Installments = sumInstallmentsByMonth(couple.person1.installments, month)
        val person2Installments = sumInstallmentsByMonth(couple.person2.installments, month)

        val person1TotalCosts = person1Costs + person1Installments
        val person2TotalCosts = person2Costs + person2Installments
        val totalCosts = person1TotalCosts + person2TotalCosts

        // Calculate contributions using the provided strategy
        val contributionsResult = strategy.calculate(person1Earnings, person2Earnings, totalCosts)
        ...
        val person1ActualContribution = person1TotalCosts
        val person2ActualContribution = person2TotalCosts

        // Calculate credits/debts
        val person1Credit =
            person1ActualContribution - person1ContributionShouldBe + couple.person1.cumulativeCredit
        val person2Credit =
            person2ActualContribution - person2ContributionShouldBe + couple.person2.cumulativeCredit
        ...
        result[month] =
            mutableMapOf(
                ...
                "person1Installments" to person1Installments,
                "person2Installments" to person2Installments,
                "person1TotalCosts" to person1TotalCosts,
                "person2TotalCosts" to person2TotalCosts,
...
fun sumCostsByMonth(costs: List<Cost>, month: YearMonth): Double {
    return costs.filter { YearMonth.from(it.date) == month }.sumOf { it.amount }
}

fun sumInstallmentsByMonth(installments: List<Installment>, month: YearMonth): Double {
    return installments.filter { it.date == month }.sumOf { it.amount }
}
...
fun printContributions(contributions: Map<YearMonth, Map<String, Any>>, couple: Couple) {
    ...
    contributions.forEach { (month, data) ->
        ...
        println("${couple.person1.name} Installments: ${data["person1Installments"]}")
        println("${couple.person2.name} Installments: ${data["person2Installments"]}")
        println("${couple.person1.name} Total Costs: ${data["person1TotalCosts"]}")
        println("${couple.person2.name} Total Costs: ${data["person2TotalCosts"]}")
